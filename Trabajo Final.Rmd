---
title: "Trabajo final de CÃ¡lculo Actuarial III"
author: "Bernardo MondragÃ³n Brozon"
date: "May 12, 2018"
output: pdf_document 
---
...
La severidad total estÃ¡ dada por lo siguiente:
$$ S=x_1+x_2+\cdots+x_N  $$
En donde $\{x_i\}_{i=1\dots N}$ son variables aleatorias independientes e idÃ©nticamente distribuidas que indican los montos tal que $x_i$ sigue una distribuciÃ³n exponencial con media $1/\lambda_{exp}=10000$ para toda $i$, y $N$ es la variable aleatoria independiente de las $x_i$'s que indica la frecuencia de los montos tal que sigue una distribuciÃ³n de Poisson con media $\lambda_{Po}=0.1$ eventos en un aÃ±o.  
...
\end{aligned}$$
En donde $F_X^{*n}(s)$ es la convoluciÃ³n de las variables aleatorias $\{x_i\}_{i=1\dots N}$, o bien, la funciÃ³n de distribucion de probabilidad acumulada de $\sum_{i=n}^{N}x_i$ con $x_i \sim exp(\lambda_{exp}=1/10000)$. Sean $M_{\sum_{i=n}^{N}x_i}(t)$ y $M_{x_1}(t)$ las funciones generadoras de momentos de las variables aleatorias $\sum_{i=n}^{N}x_i$ y $x_1$ respectivamente, entonces, sin pÃ©rdida de generalidad, se tiene que
...
Lo cual corresponde a la funciÃ³n generadora de momentos de una variable aleatoria distribuida Gamma con parÃ¡metros de $\alpha = n$ y $\beta = \lambda_{exp}$. AdemÃ¡s, la distribuciÃ³n de la frecuencia es Poisson con parÃ¡metro $\lambda_{Po}=0.1$, entonces se sigue que
...
# Title
title <- ggtitle("FunciÃ³n de probabilidad acumulada \n de la severidad total")
...
La funciÃ³n de densidad de probabilidad de la severidad total esta dada por
$$\begin{aligned}
...
# Title
title.pdf <- ggtitle("FunciÃ³n de densidad de probabilidad \n de la severidad total")
...
A continuaciÃ³n se presenta una tabla que indica la probabilidad que se acumula en ciertos puntos de la distribuciÃ³n de la severidad total:
...
\section{1.2 SimulaciÃ³n}

Para hacer simulaciones de la severidad total, primero hay que simular el nÃºmero $N$ de siniestros ocurridos, el cual proviene de la distribuciÃ³n de Poisson con media $\lambda_{Po}=0.1$. Una vez simulado el nÃºmero $N$ de sinistros, se simulan $N$ siniestros, que son variables aleatorias exponenciales con parÃ¡metro $\lambda_{exp}=1/10000$. DespuÃ©s de obtener los $N$ sinistros, se suman y de esta manera se obtiene un primer valor de la severidad total. Para concluir con la simulaciÃ³n, se repite el proceso anterior $10000$ veces. A continuaciÃ³n se muestran los primeros $100$ valores obtenidos de la simulaciÃ³n:
...
El histograma debe parecerse a la funciÃ³n de densidad de probabilidad (la linea punteada indica la seveidad promedio):
...
Se vio que la severidad total, que sigue uns distribuciÃ³n de Poisson Compuesta, estÃ¡ dada por
$$  S=\sum_{i=1}^{N}x_i.$$
Entonces el valor esperado de la severidad total estÃ¡ dado por
$$\begin{aligned}
...
La funciÃ³n generadora de momentos de $S$ esta dada por
...
se sigue que el sesgo de la distribuciÃ³n es positivo y estÃ¡ dado por
...
y el coeficiente de Kurtosis estÃ¡ dado por
...
# Legends
an <- labs(TeX('Severidad total'), TeX('Aproximación Gamma'))
# Adjust scales on axes
xlim <- xlim(0,80000)
ylim <- ylim(0,.00001)
# Plot
...
La distribución empirica de la severidad totoal está dada por
...
ggplot() + stat_ecdf(data=df, aes(x=severities), geom = "step") +
  labs(x = TeX('$s$'), y = TeX('$F_{S_n}(s)$')) +
  ggtitle("Distribución empírica de la severidad total") + theme + ylim(0.857,1)
```
...
Si se construyen los intervalos de confianza al $95\%$ para $F_S(s)$ utilizando la distribución empirica se tiene lo siguiente:
...
title <- ggtitle("Intervalos de confianza para \n la distribucion de la severidad total")
...
p+theme+xAxis+yAxis+labels+title+cdf.layer+fun1+fun2+fun3+xlim+ylim+
scale_colour_manual(
  #name="Name",
  name = NULL,
  values = colors,
