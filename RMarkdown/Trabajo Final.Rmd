...
author: "Bernardo Mondragon Brozon, Anhara Rueda"
date: "May 12, 2018"
output: pdf_document
---
...
El histograma debe parecerse a la funcion de densidad de probabilidad (la linea punteada indica la seveidad promedio):
...
\section{1.3 Comparacion de los valores reales con los estimados}
...
\section{1.4 Aproximacion de la distribucion de la severidad total}
...
Esta proximacion Lognormal tiene un corrimiento igual $5000$, una media de $4$ y una desviacion estandar de $5$.


\section{1.3 distribucion empirica}
...
La linea azul claro indica la media de la distribucion teorica a partir de la cual fueron simulados los datos.
...
$$  E\left( \min{M, l} \right)=\int_{0}^{l}\frac{m}{10000}e^{-\frac{m}{10000}} \, \mathrm{d}m=10000-(l+10000)e^{-\frac{l}{10000}} $$
...
Como los montos son distribuidos exponencialmente con media $E(x_1)=1/\lambda_{exp}$, no se observan indicios de una distribucio de cola pesada.
...
points <- geom_point(
  data=data.frame(severities2, media_exceso, type="1"),
  aes(x=severities2, y=media_exceso, colour=type, linetype=type, size=type),
  shape=1
)
...
points <- geom_point(
  data=data.frame(severities2, mort_emp, type="1"),
  aes(x=severities2, y=mort_emp, colour=type, linetype=type, size=type),
  shape=1
)
...
p.m+theme+xAxis+yAxis+labs(x="Montos", y="Mortalidad")+title+curve+points+xlim(0,80000)+ylim(0,1)+ asinth.axis +
scale_colour_manual(
  #name="Name",
  name = NULL,
  values = colors,
...
\subsection{2.6 Prueba de bondad de ajuste Ji-cudrado}
...
xlim(0,40) +
ylim(0,0.1) +
  labs(x = TeX('$x$'), y = TeX('$ f_{\\chi_{(14)}^2}(x) $')) + xAxis + yAxis + ggtitle("Prueba Ji-Cuadrado") + theme +
...
Por lo tanto la hipotesis de que los montos de los siniestros siguen una distribucion exponencial con media $10000$ no es rechazada.

Que es menor que el punto crítico de una Jí-cuadrada con 13 grados de libertad: $$PC=`r test`$$
...
\section{3.1 Estimacion con censura y truncamiento}

Sobre los 10000 siniestros simulados se considera que aplicarán un deducible de $5,000 y un límite de $30,000. El archivo de indemnizaciones queda de la siguiente manera: 
...
\section{3.2 Estimacion por máxima verosimilitud del parámetro de la distribución exponencial con la censura y el truncamiento}
